...
./BurpNinja.sh          # macOS: standard user, do NOT use sudo
...
- **`test_*` helpers `exit` the whole program, not just the action.**
  `test_adb`, `test_burpsuite`, `test_internet`, and the root check call
  `exit 1` on failure. Selecting option `7` with no device prints
  `[!] ADB device not ready: error: no devices/emulators found` and the
  script ends — it does **not** loop back to the menu. Fix the precondition,
  then relaunch.
...
| Script exits right after choosing 2/4/5/6/7/8 | A `test_*` precondition failed (no device / Burp down / not root). Run `driver.sh doctor`, fix the red line, retry. |
